...
aerosols = ['oc', 'ec', 'pno3', 'pso4', 'pmfine', 'pmc', 'poa']
pmc_list = ['pmc', 'PMC']
rline_shp = False
...
class Traffic(object):
    ...
    :param road_link_path: Path to the file that contains all the road links information as the geometry of each road.
    :type road_link_path: str

    :param fleet_compo_path: ...
    :type fleet_compo_path: str

    :param speed_hourly_path: ...
    :type speed_hourly_path: str

    :param monthly_profile_path: Path to the file that contains all the monthly profiles needed.
    :type monthly_profile_path: str

    :param daily_profile_path: Path to the file that contains all the daily profiles needed.
    :type daily_profile_path: str

    :param hourly_mean_profile_path: Path to the file that contains the hourly means profiles needed. It will be used
        when no other hourly profile is set or selected.
    :type hourly_mean_profile_path: str

    :param hourly_weekday_profile_path: Path to the file that contains the weekday hourly profiles needed.
    :type hourly_weekday_profile_path: str

    :param hourly_saturday_profile_path: Path to the file that contains the Saturday hourly profiles needed.
    :type hourly_saturday_profile_path: str

    :param hourly_sunday_profile_path: Path to the file that contains the Sunday hourly profiles needed.
    :type hourly_sunday_profile_path: str

    :param ef_common_path: Path to the folder that contains all the emission factor files. The name of these files are
        hardcoded like '<emission_type>_<pollutant>.csv' and also the correction files 'mcorr_<pollutant>.csv'
    :type ef_common_path: str

    :param pollutants_list: List of pollutatnts to take into account for the hot & cold emissions. The other ones are
        hardcoded to be only PM10 and PM2.5.
    :type pollutants_list: list

    :param date: Starting date (UTC) for the firts timestep.
    :type date: datetime.datetime

    :param load: Load of the heavy vehicles. this value can be empty (0.0) medium (0.5) or full (1.0)
    :type load: float

    :param timestep_type: Increment between timestep must to be 'hourly' for the traffic bottom-up emissions.
    :type timestep_type: str

    :param timestep_num: number of timesteps to simulate
    :type timestep_num: int

    :param timestep_freq: Frequency between timesteps.
    :type timestep_freq: int

    :param temp_common_path: Path to the auxiliar folder to store the intermediate files needed.
    :type temp_common_path: str
    """

    def __init__(self, auxiliar_path, clipping, road_link_path, fleet_compo_path, speed_hourly_path,
                 ...
                 molecular_weights_path=None):
        try:
            settings.log_level_3
        except AttributeError:
            settings.define_global_vars(0)
    ...
    def read_temperature(lon_min, lon_max, lat_min, lat_max, temp_dir, date, tstep_num, tstep_freq):
        ...
        i_min, i_max, j_min, j_max = Traffic.find_temperature_index(lon_o, lat_o, lon_min, lon_max, lat_min, lat_max)
    ...
    @ staticmethod
    def find_temperature_index(lon, lat, lon_min, lon_max, lat_min, lat_max):
    ...
    def calculate_hot(self):
        ...
        for pollutant in self.pollutant_list:
            if pollutant != 'nh3':
                ...
                expanded_aux = pd.concat([df_code_slope_road, df_code_slope, df_code_road, df_code])#.set_index('index')
    ...
    def calculate_cold(self, hot_expanded):
        ...
        for tstep in xrange(self.timestep_num):
            ...
            if 'voc' in self.pollutant_list and 'ch4' in self.pollutant_list:
                ...
                del cold_df['voc_{0}'.format(tstep)], cold_df['ch4_{0}'.format(tstep)]
            else:
    ...
    def compact_hot_expanded(self, expanded):
        ...
        for tstep in xrange(self.timestep_num):
            ...
            if 'voc' in self.pollutant_list and 'ch4' in self.pollutant_list:
                ...
                del expanded['voc_{0}'.format(tstep)], expanded['ch4_{0}'.format(tstep)]
            else:
                warnings.warn(
    ...
    def calculate_resuspension(self):
        ...
        for pollutant in pollutants:
            ...
            for tstep in xrange(self.timestep_num):
                ...
                # TODO Calculate PR for each tstep
                df.loc[:, pr_column] = 1
