...
class AgriculturalSector(Sector):
    ...
    def get_land_use_by_nut_csv(self, land_use_distribution_src_nut, land_uses):
        ...
        #      np.unique(land_use_distribution_src_nut['land_use'].astype(np.int16))], names=['nut_code', 'land_use']))

        land_use_distribution_src_nut['area'] = land_use_distribution_src_nut.area
        land_use_by_nut = land_use_distribution_src_nut.groupby(['nut_code', 'land_use'])['area'].sum()
    ...
    def get_crops_by_dst_cell(self, file_path):
        ...
        if not os.path.exists(file_path):
            ...
            print('land_use_by_nut', land_use_by_nut.sum())
            sys.stdout.flush()
            ...
            print('crop_by_nut', crop_by_nut.sum())
            print('tot_crop_by_nut', tot_crop_by_nut.sum())
            sys.stdout.flush()
            exit()
