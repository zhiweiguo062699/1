...
class AgriculturalCropFertilizersSector(AgriculturalSector):
    def __init__(self, auxiliary_dir, grid_shp, clipping, date_array, nut_shapefile, source_pollutants, crop_list,
        ...
        print '\nRank: {0}, {1} {2} {3}'.format(settings.rank, len(self.crop_distribution), len(self.gridded_constants), len(self.ef_by_crop))
        ...
        print 'TIME -> AgriculturalCropFertilizers.__init__: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
    ...
    def get_ftype_fcrop_fmode_by_nut(self, crop, nut_list):
        ...
        print 'TIME -> AgriculturalCropFertilizers.get_ftype_fcrop_fmode_by_nut: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
    ...
    def get_ef_by_crop(self):
        ...
        for crop in self.element_list:
            ...
            if formula:
                ...
                crop_ef['f_ph'] = (0.067 * self.gridded_constants['ph'] ** 2) - (0.69 * self.gridded_constants['ph']) + 0.68
            else:
                crop_ef['f_ph'] = 0
                crop_ef.loc[self.gridded_constants['ph'] <= 5.5, 'f_ph'] = -1.072
                crop_ef.loc[(self.gridded_constants['ph'] > 5.5) & (self.gridded_constants['ph'] <= 7.3), 'f_ph'] = -0.933
                crop_ef.loc[(self.gridded_constants['ph'] > 7.3) & (self.gridded_constants['ph'] <= 8.5), 'f_ph'] = -0.608
            ...
            crop_ef['f_sum'] = np.exp(crop_ef['f_ph'] + crop_ef['f_cec'] + crop_ef['f_type'] + crop_ef['f_crop'] + crop_ef['f_mode'])

            total_crop_df['EF_{0}'.format(crop)] = crop_ef['f_sum']
        print 'TIME -> AgriculturalCropFertilizers.get_ef_by_crop: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
    ...
    def to_dst_resolution(self, src_shapefile, value):
        ...
        intersection_with_dst_areas = pd.merge(intersection, dst_shapefile.loc[:, ['FID', 'involved_area']], how='left', on='FID')
        intersection_with_dst_areas['involved_area'] = intersection_with_dst_areas['area'] / intersection_with_dst_areas['involved_area']

        intersection_with_dst_areas[value] = intersection_with_dst_areas[value] * intersection_with_dst_areas['involved_area']
        ...
        print 'TIME -> AgriculturalCropFertilizers.to_dst_resolution: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
    ...
    def get_gridded_constants(self, gridded_ph_cec_path, ph_path, clipped_ph_path, cec_path, clipped_cec_path):
        ...
        print 'TIME -> AgriculturalCropFertilizers.get_gridded_constants: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
    ...
    def get_daily_inputs(self, yearly_emissions):
        ...
        for day in self.day_dict.keys():
            ...
            for crop in self.element_list:
                meteo_df['d_{0}'.format(crop)] = self.get_data_from_netcdf(
                    self.fertilizer_denominator_yearly_factor_path.replace('<crop>', crop).replace('<year>', str(day.year)),
        ...
        print 'TIME -> AgriculturalCropFertilizers.get_daily_inputs: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
    ...
    def calculate_yearly_emissions(self):
        ...
        print 'TIME -> AgriculturalCropFertilizers.calculate_yearly_emissions: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
    ...
    def calculate_nh3_emissions(self, day, daily_inputs):
        ...
        for crop in self.element_list:
            ...
            try:
                thau_2 = float(thau_2)
                sum = (beta_1 / (sigma_1 * math.sqrt(2 * math.pi))) * math.exp((float(int(day.strftime('%j')) - thau_1) ** 2) / (-2 * (sigma_1 ** 2)))
                sum += (beta_2 / (sigma_2 * math.sqrt(2 * math.pi))) * math.exp((float(int(day.strftime('%j')) - thau_2) ** 2) / (-2 * (sigma_2 ** 2)))
                sum += (beta_3 / (sigma_3 * math.sqrt(2 * math.pi))) * math.exp((float(int(day.strftime('%j')) - thau_3) ** 2) / (-2 * (sigma_3 ** 2)))
            except ValueError:
                aux = (beta_1 / (sigma_1 * math.sqrt(2 * math.pi))) * math.exp((float(int(day.strftime('%j')) - thau_1) ** 2) / (-2 * (sigma_1 ** 2)))
                aux += (beta_3 / (sigma_3 * math.sqrt(2 * math.pi))) * math.exp((float(int(day.strftime('%j')) - thau_3) ** 2) / (-2 * (sigma_3 ** 2)))
                sum = (beta_2 / (sigma_2 * math.sqrt(2 * math.pi))) * np.exp(((int(day.strftime('%j')) - daily_inputs[thau_2]).astype(np.float64)) ** 2 / (-2 * (sigma_2 ** 2)))
                sum += aux
            daily_inputs['FD_{0}'.format(crop)] = daily_inputs['exp'].multiply(sum)
            # daily_inputs.loc[:, ['FID', 'FD_{0}'.format(crop)]].to_csv(os.path.join(self.auxiliary_dir, 'fertilizers', 'FD_{0}_{1}.csv'.format(crop, day.strftime('%Y%m%d'))))

        for crop in self.element_list:
            daily_inputs[crop] = daily_inputs[crop].multiply(daily_inputs['FD_{0}'.format(crop)] / daily_inputs['d_{0}'.format(crop)])
        ...
        print 'TIME -> AgriculturalCropFertilizers.calculate_nh3_emissions: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
    ...
    def add_dates(self, df_by_day):
        ...
        print 'TIME -> AgriculturalCropFertilizers.add_dates: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
    ...
    def calculate_daily_emissions(self, emissions):
        ...
        for day, daily_inputs in df_by_day.iteritems():
            ...
            # df_by_day[day].groupby('nut_code')['nh3'].sum().to_csv(os.path.join(self.auxiliary_dir, 'fertilizers', '{0}.csv'.format(day.strftime('%Y%m%d'))))
        print 'TIME -> AgriculturalCropFertilizers.calculate_daily_emissions: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
    ...
    def calculate_hourly_emissions(self, emissions):
        ...
        print 'TIME -> AgriculturalCropFertilizers.calculate_hourly_emissions: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
        return emissions
    ...
    def calculate_emissions(self):
        ...
        print 5
        # emissions = self.get
        print 'TIME -> AgriculturalCropFertilizers.calculate_emissions: Rank {0} {1} s'.format(settings.rank, round(gettime() - st_time, 2))
